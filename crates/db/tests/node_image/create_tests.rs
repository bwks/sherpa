/// CREATE operation tests for node_image
use anyhow::Result;
use db::{create_node_image, list_node_images, upsert_node_image};
...
async fn test_create_and_list_node_images() -> Result<()> {
    let db = setup_db("test_create_and_list_node_images").await?;

    // Create a test config
    let test_config = create_test_config(NodeModel::AristaVeos);
    let created = create_node_image(&db, test_config.clone()).await?;

    assert_eq!(created.model, NodeModel::AristaVeos);
    assert!(created.id.is_some(), "Created config should have an ID");

    // List all configs
    let all_configs = list_node_images(&db).await?;
    assert!(
...
#[tokio::test]
#[ignore] // Requires running SurrealDB instance
async fn test_upsert_node_image() -> Result<()> {
    let db = setup_db("test_upsert_node_image").await?;

    // First upsert - should create
    let test_config = create_test_config(NodeModel::NokiaSrlinux);
    let first_upsert = upsert_node_image(&db, test_config.clone()).await?;
    ...
    let second_upsert = upsert_node_image(&db, test_config).await?;
