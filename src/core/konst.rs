pub const CONFIG_FILE: &str = "sherpa.toml";
pub const MANIFEST_FILE: &str = "manifest.toml";
pub const CONFIG_DIR: &str = "~/.sherpa";
pub const BOXES_DIR: &str = "boxes";
pub const QEMU_BIN: &str = "/usr/bin/qemu-system-x86_64";
pub const QEMU_URI: &str = "qemu:///system";
pub const KVM_OUI: &str = "52:54:00";
pub const ARISTA_OUI: &str = "02:01:00";
pub const CISCO_IOSXE_OUI: &str = "02:02:00";
pub const CISCO_IOSV_OUI: &str = "02:02:01";
pub const JUNIPER_OUI: &str = "02:03:00";
pub const CUMULUS_OUI: &str = "02:04:00";
pub const BASE_PORT: u16 = 10000;
pub const TELNET_PORT: u16 = 13337;
pub const STORAGE_POOL: &str = "default";
pub const STORAGE_POOL_PATH: &str = "/var/lib/libvirt/images";
pub const SHERPA_STORAGE_POOL: &str = "sherpa-pool";
pub const SHERPA_STORAGE_POOL_PATH: &str = "/var/lib/libvirt/sherpa";
pub const BOOT_NETWORK_NAME: &str = "sherpa-bootnet";
pub const BOOT_NETWORK_BRIDGE: &str = "sherpa-br0";
pub const BOOT_NETWORK_IP: &str = "192.168.128.1";
pub const BOOT_NETWORK_PORT: u16 = 13337;
pub const BOOT_NETWORK_NETMASK: &str = "255.255.255.0";
pub const BOOT_NETWORK_DHCP_START: &str = "192.168.128.10";
pub const BOOT_NETWORK_DHCP_END: &str = "192.168.128.254";
pub const BOOT_NETWORK_HTTP_SERVER: &str = "192.168.128.1";
pub const ISOLATED_NETWORK_NAME: &str = "sherpa-blackhole";
pub const ISOLATED_NETWORK_BRIDGE: &str = "sherpa-br666";
pub const ZTP_ISO: &str = "ztp.iso";
pub const ZTP_JSON: &str = "ztp.json";
pub const CLOUD_INIT_USER_DATA: &str = "user-data";
pub const CLOUD_INIT_META_DATA: &str = "meta-data";
pub const _SSH_DIR: &str = "~/.ssh";
pub const _SSH_PUBLIC_KEY_FILE: &str = "id_rsa.pub";
pub const TEMP_DIR: &str = ".tmp";
pub const _MTU_STD: u16 = 1500;
pub const MTU_JUMBO_INT: u16 = 9126;
pub const MTU_JUMBO_NET: u16 = 9600;
pub const DOMAIN_NAME: &str = "sherpa.lab.local";
pub const ZTP_DIR: &str = "ztp";
pub const CISCO_ZTP_DIR: &str = "cisco";
pub const CISCO_IOSXE_ZTP_CONFIG: &str = "iosxe-config.txt";
pub const CISCO_IOSV_ZTP_CONFIG: &str = "iosv-config.txt";
pub const CUMULUS_ZTP_DIR: &str = "cumulus";
pub const CUMULUS_ZTP_CONFIG: &str = "cumulus-ztp";
pub const ARISTA_ZTP_DIR: &str = "arista";
pub const ARISTA_VEOS_ZTP_CONFIG: &str = "veos-config.txt";
pub const SHERPA_USERNAME: &str = "sherpa";
pub const SHERPA_PASSWORD: &str = "Gang!42069";
pub const SHERPA_SSH_PUBLIC_KEY_FILE: &str = "sherpa_ssh_key.pub";
pub const SHERPA_SSH_PRIVATE_KEY_FILE: &str = "sherpa_ssh_key";
pub const SHERPA_SSH_CONFIG_FILE: &str = "sherpa_ssh_config";
pub const SSH_PORT: u16 = 22;
pub const READINESS_TIMEOUT: u64 = 600;
pub const READINESS_SLEEP: u64 = 10;
pub const ZTP_USB: &str = "usb-ztp.qcow2";
pub const IGNITION_VERSION: &str = "2.3.0";
pub const IGNITION_CONFIG_DIR: &str = "/var/lib/libvirt/flatcar-linux/";
